...
function VanguardSectionTitle({
  ...
  return (
    <div className="space-y-1.5 mt-5 mb-3 break-inside-avoid">
      ...
      <div className={cn("h-[3px] w-full rounded-sm", accentColor)} />
    </div>
  );
}
...
export function VanguardTemplate({
  ...
  return (
    <div className="space-y-5 font-sans text-neutral-800 max-w-[800px] mx-auto p-2 leading-normal">
      ...
      <header className="flex justify-between items-start gap-4 pb-3 border-b border-neutral-100">
        <div className="space-y-2 flex-1">
          ...
          {data.jobTitle?.length && (
            <p className={cn("text-xs font-bold tracking-wider uppercase", textColor)}>
              {data.jobTitle?.join?.(" | ")}
            </p>
          )}
        ...
        {template.supportsPhoto && data.photoUrl && (
          <img
            ...
            className={cn("h-20 w-20 rounded-full object-cover shrink-0 border-2 shadow-md", borderColor)}
          />
        )}
      </header>
          ...
          if (section.id === "summary" && data.summary) {
            return (
              <section key="summary" className="space-y-2">
                <VanguardSectionTitle label="Summary" textColor={textColor} accentColor={accentColor} />
          ...
          if (section.id === "experience" && data.experience.length > 0) {
            return (
              <section key="experience" className="space-y-1">
                <VanguardSectionTitle label="Experience" textColor={textColor} accentColor={accentColor} />
                <div className="space-y-3">
                  {data.experience.map((item, i) => (
                    <div key={i} className="space-y-1.5 break-inside-avoid">
                      {i > 0 && <div className="border-t border-dashed border-neutral-200 my-2" />}
                      <div>
                        <p className="text-sm font-bold text-neutral-900">{item.position}</p>
                        <p className={cn("text-xs font-bold mt-0.5", textColor)}>
                          {item.company}
                        </p>
                      </div>
                      ...
                      <RichText html={item.description} className="text-neutral-700 text-xs leading-relaxed pt-0.5" />
                    </div>
                  ))}
                </div>
              </section>
          ...
          if (section.id === "education" && data.education.length > 0) {
            return (
              <section key="education" className="space-y-1">
                <VanguardSectionTitle label="Education" textColor={textColor} accentColor={accentColor} />
                <div className="space-y-3">
                  {data.education.map((item, i) => (
                    <div key={i} className="space-y-1.5 break-inside-avoid">
                      ...
                      {item.description && <RichText html={item.description} className="text-neutral-700 text-xs mt-1" />}
                    </div>
                  ))}
                </div>
              </section>
          ...
          if (section.id === "skills" && data.skills.length > 0) {
            return (
              <section key="skills" className="space-y-2">
                <VanguardSectionTitle label="Skills" textColor={textColor} accentColor={accentColor} />
          ...
          if (section.id === "languages" && data.languages.length > 0) {
            return (
              <section key="languages" className="space-y-2">
                <VanguardSectionTitle label="Languages" textColor={textColor} accentColor={accentColor} />
                <div className="flex flex-wrap gap-2">
                  {data.languages.map((lang, i) => (
                    ...
                    >
                      <span>{lang.name}</span>
                      {lang.level && <span className="text-[10px] text-neutral-400 font-normal">({lang.level})</span>}
                    </span>
                  ))}
                </div>
              </section>
          ...
          if (section.id === "projects" && data.projects.length > 0) {
            return (
              <section key="projects" className="space-y-2">
                <VanguardSectionTitle label="Projects" textColor={textColor} accentColor={accentColor} />
                <div className="grid grid-cols-2 gap-4">
                  {data.projects.map((item, i) => (
                    <div key={i} className="space-y-1.5 p-3 rounded border border-neutral-100 shadow-sm break-inside-avoid">
          ...
          if (section.id === "certifications" && data.certifications.length > 0) {
            return (
              <section key="certifications" className="space-y-2">
                <VanguardSectionTitle label="Certifications" textColor={textColor} accentColor={accentColor} />
          ...
          if (section.id === "awards" && data.awards.length > 0) {
            return (
              <section key="awards" className="space-y-2">
                <VanguardSectionTitle label="Key Achievements" textColor={textColor} accentColor={accentColor} />
                <div className="grid grid-cols-2 gap-4">
          ...
          if (section.id.startsWith("other-field-")) {
            ...
            return (
              <section key={section.id} className="space-y-2">
                <VanguardSectionTitle label={(section as any).title} textColor={textColor} accentColor={accentColor} />
