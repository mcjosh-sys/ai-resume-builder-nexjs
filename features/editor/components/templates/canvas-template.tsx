...
function MercadoSidebarHeading({ label, accent }: { label: string; accent?: string }) {
  return (
    <div className="flex items-center gap-1.5 mt-3 mb-1.5 first:mt-0">
      <span className={cn("h-1 w-1 rounded-full bg-white opacity-60 shrink-0")} />
...
function MercadoMainHeading({ label, accent }: { label: string; accent: string }) {
  return (
    <div className="mb-2 flex items-center gap-2">
      <p className={cn("text-xs font-bold uppercase tracking-widest", accent.replace("bg-", "text-"))}>
        {label}
      </p>
      <div className={cn("flex-1 h-px", accent.replace("bg-", "bg-") + " opacity-20")} />
    </div>
  );
}
      ...
      >
        ...
        {fullName && (
          <div className="mt-1">
            ...
            {data.jobTitle && (
              <p className="text-[11px] text-white opacity-70 mt-0.5">{data.jobTitle}</p>
            )}
          </div>
        )}
        ...
        <div className="space-y-1.5">
          {data.phone && (
            <div className="space-y-0">
              <p className="text-[10px] text-white text-opacity-60 uppercase tracking-wide">Phone</p>
              <p className="text-xs text-white">{data.phone}</p>
            </div>
          )}
          {data.email && (
            <div className="space-y-0">
              <p className="text-[10px] text-white text-opacity-60 uppercase tracking-wide">Email</p>
              <p className="text-xs text-white break-all">{data.email}</p>
            </div>
          )}
          {location(data.city, data.country) && (
            <div className="space-y-0">
              <p className="text-[10px] text-white text-opacity-60 uppercase tracking-wide">Address</p>
          ...
          if (section.id.startsWith("other-field-")) {
            ...
            return (
              <div key={section.id}>
                ...
                <p className="text-xs font-semibold text-white">{fieldData.title}</p>
                {fieldData.subtitle && (
                  <p className="text-xs text-white text-opacity-70">{fieldData.subtitle}</p>
                )}
                <RichText html={fieldData.description} className="text-white text-xs" />
              </div>
            );
          }
